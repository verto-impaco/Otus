...
def test_log_product_created_task_outputs_product_name(product, capsys):
    result = log_product_created.delay(product.pk, product.name)

    output = capsys.readouterr().out
    assert result.successful()
    assert product.name in output
    assert f"ID: {product.pk}" in output
